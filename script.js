...
let DIES = localStorage.getItem("DIES") || 0
...
function Jump() {
    if (!lose2) {
        ...
        if (bottomValue > 300) {
            ...
            if (canReset) {
                DIES = Number(DIES) + 1;
                localStorage.setItem('DIES', DIES);
...
function Die(audio2) {
    if (!canAudio) {
        canAudio = true;
        let audioFile = audio2 === 1 ? 'hit.wav' : 'die.wav';
...
let time3 = setInterval(() => {
    ...
    document.getElementById('TextDie').textContent = DIES
